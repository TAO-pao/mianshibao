...
import com.taoxinyu.mianshibao.manager.CounterManager;
import com.taoxinyu.mianshibao.model.dto.question.*;
import com.taoxinyu.mianshibao.model.entity.Question;
import com.taoxinyu.mianshibao.model.entity.QuestionBankQuestion;
import com.taoxinyu.mianshibao.model.entity.User;
import com.taoxinyu.mianshibao.model.vo.QuestionVO;
import com.taoxinyu.mianshibao.service.QuestionBankQuestionService;
...
public class QuestionController {
    ...
    public BaseResponse<Page<QuestionVO>> searchQuestionVOByPage(
        ...
        try {
            ...
            // 若ES查询无结果，可选择是否降级到数据库（根据业务需求）
            if (questionPage == null || questionPage.getTotal() == 0) {
                log.warn("ES查询无结果，降级为数据库查询");
                questionPage = questionService.listQuestionByPage(questionQueryRequest);
            }
        } catch (Exception e) {
            // ES查询失败（如索引不存在、连接超时等），降级到数据库
            log.error("ES查询异常，降级为数据库查询", e);
            questionPage = questionService.listQuestionByPage(questionQueryRequest);
        }
